...
def render_charts(df: pd.DataFrame, vectorizer) -> None:
    ...
    try:
        # Vibrant palette for discrete categorical data
        category_palette = ["#6366f1", "#10b981", "#f59e0b", "#ef4444", "#8b5cf6", "#06b6d4"]
        ...
        fig1.update_layout(
            ...
            font=dict(family="DM Sans", color="#0f172a", size=13),
            title_font_size=18,
        ...
        fig2 = px.bar(
            ...
            color=counts,
            color_continuous_scale=["#e0e7ff", "#4f46e5"],  # Soft indigo gradient
            text=counts
        )
        fig2.update_layout(
        ...
        fig3.update_traces(
            marker=dict(size=8, line=dict(color="#ffffff", width=1), opacity=0.85)
        )
